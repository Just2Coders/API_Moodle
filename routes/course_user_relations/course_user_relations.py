...
async def get_completed_courses(user_id: Annotated[str,Depends(find_userid)]):
    ...
    for course in user_courses_serialized:
        ...
        print(course_completion_status_serealized)
# Se verifica si se devuelve alguna expecion por moodle en caso de que este blockeado u otro escenario y si es ocurre
# se continua al siguiente curso
        if course_completion_status.status_code == 200 and course_completion_status_serealized["exception"] == "moodle_exception" :
            continue 
        else:
            # si el curso esta completado se agrega a la lista de cursos completados
            if course_completion_status_serealized['completionstatus']['completed']:
                completed_courses.append(
                    {"fullname":course["fullname"],
                     "id":course["id"]})          
    for course in completed_courses:
        #luego se recorre la lista d ecursos completados y se obtiene todas las actividades completadas de ese curso
...
@course_user_router.get("/Obtener_archivos_by_tokenuser",summary="Devuelve la informacion general de un curso asi como el contenido del mismo")
@error_handler
async def obtener_archivos_single_by_token(courseid: int,
                                           Token:Annotated[str,Depends(oauth2_scheme)],
                                           moodlewsrestformat:Annotated[str,Header()]="json"):      
